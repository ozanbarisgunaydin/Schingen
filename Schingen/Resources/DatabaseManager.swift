...
extension DatabaseManager {
    ...
    public func  insertUser(with user: SchingenUser, completion: @escaping (Bool) -> Void) {
...
extension DatabaseManager {
    ...
    /// Create a new conversation with the chosen user email and first message send.
    public func createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        referance.observeSingleEvent(of: .value, with: { [weak self] snapshot in
...
//                Conversation array exists for current user. You should append!
                ...
                referance.setValue(userNode, withCompletionBlock: {[weak self] error, _ in
                    guard error == nil else {
                        completion(false)
                        return
                    }
...
//                Location
                    ...
                    let location = Location(location: CLLocation(latitude: longitude, longitude: latitude), size: CGSize(width: 300, height: 300))
...
//        Update recipient latests message.
        ...
        let currentEmail = DatabaseManager.safeEmail(emailAddress: ownerEmail)
...
//                                    Failed to find incurrent collection.
                                    let newConversationData: [String : Any] = [
                                        "id" : conversation,
...
//        Reset those conversations for the user in Firebase Database.
        let refForConversations = database.child("\(safeEmail)/conversations")
        refForConversations.observeSingleEvent(of: .value) { snapshot in
            if var conversations = snapshot.value as? [[String : Any]] {
                ...
                refForConversations.setValue(conversations, withCompletionBlock: { error, _ in
                    guard error == nil else {
                        completion(false)
